...
import type { TransformationType } from "@/modules/styles";
...
// Room types with icons - matches database room_specifications
const roomTypes = [
  // Pièces de vie
  ...
  // Chambres
  { value: "chambre" as RoomType, label: "Chambre Principale", icon: BedDouble },
  { value: "chambre_enfant" as RoomType, label: "Chambre d'Enfant", icon: Baby },

  // Pièces d'eau
  { value: "salle_de_bain" as RoomType, label: "Salle de Bain", icon: Bath },
  { value: "wc" as RoomType, label: "WC", icon: DoorOpen },
  { value: "toilette" as RoomType, label: "Toilette (avec lavabo)", icon: Droplet },

  // Travail et rangement
  { value: "bureau" as RoomType, label: "Bureau", icon: Briefcase },
  { value: "dressing" as RoomType, label: "Dressing", icon: Shirt },
  { value: "buanderie" as RoomType, label: "Buanderie", icon: WashingMachine },
  ...
  // Espaces extérieurs
  { value: "terrasse" as RoomType, label: "Terrasse", icon: Sun },
  { value: "balcon" as RoomType, label: "Balcon", icon: Building },
  { value: "jardin" as RoomType, label: "Jardin", icon: Trees },
  { value: "veranda" as RoomType, label: "Véranda", icon: Sun },

  // Stockage
  { value: "garage" as RoomType, label: "Garage", icon: Car },
  { value: "cave" as RoomType, label: "Cave", icon: Archive },
  { value: "grenier" as RoomType, label: "Grenier", icon: Home },

  // Spéciaux
  { value: "mezzanine" as RoomType, label: "Mezzanine", icon: Layers },
  { value: "salle_de_jeux" as RoomType, label: "Salle de Jeux", icon: Gamepad2 },
  { value: "autre" as RoomType, label: "Autre", icon: MoreHorizontal },
];

// 🔧 FIX: Maximum files per upload to prevent DoS
...
export function ImageUploader({ onUploadComplete, isUploading = false }: ImageUploaderProps) {
  ...
  const [roomSearchQuery, setRoomSearchQuery] = useState(""); // 🔍 Recherche de pièce
  ...
  const processFiles = async (newFiles: File[]) => {
    // 🔧 FIX: Check maximum file count to prevent DoS
    if (files.length + newFiles.length > MAX_FILES_PER_UPLOAD) {
      toast.error(`Limite d'upload atteinte`, {
        description: `Maximum ${MAX_FILES_PER_UPLOAD} images par upload. Vous avez déjà ${files.length} image(s).`,
        duration: 5000,
      });
      return;
    }
    ...
    const validFiles = newFiles.filter((file) => {
      ...
      if (!isFileSizeValid(file, MAX_FILE_SIZE_MB)) {
        toast.error(`Fichier trop volumineux: ${file.name}`, {
          description: `Taille: ${formatFileSize(file.size)}. Maximum: ${MAX_FILE_SIZE_MB} MB`,
        });
        return false;
      }
        ...
        validFiles.map(async (file) => {
          try {
            ...
            return {
              ...
              strength: 0.15, // 🎚️ Valeur par défaut de l'intensité IA
  ...
  const removeFile = (id: string) => {
    setFiles((prev) => {
      // 🔧 FIX: Revoke blob URL before removing file
  ...
  const applyBulkStrength = (strength: number) => {
    setFiles((prev) =>
      prev.map((f) => ({ ...f, strength }))
    );
  };
          ...
          {bulkMode && (
            <>
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className="pl-10 pr-4 py-2 w-full border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200"
                    />
                    {roomSearchQuery && (
                      <button
                  ...
                  {(() => {
                    const filteredRooms = roomTypes.filter((room) =>
                      room.label.toLowerCase().includes(roomSearchQuery.toLowerCase()) ||
                      room.value.toLowerCase().includes(roomSearchQuery.toLowerCase())
                    );
              ...
              <Card className="modern-card p-6">
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-md bg-gradient-to-br from-blue-500 to-indigo-500 flex items-center justify-center flex-shrink-0">
                      <Sparkles className="text-white" size={20} />
                    </div>
                    <div className="flex-1">
                      <Label className="text-base text-slate-900 font-bold block mb-1">
                        Intensité de la transformation IA
                      </Label>
                      <p className="text-sm text-slate-600">
                        Contrôlez à quel point l'IA transforme vos images
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium text-slate-700">
                        Strength: {((files[0]?.strength ?? 0.15) * 100).toFixed(0)}%
                      </span>
                    ...
                    <Slider
                      value={[files[0]?.strength ?? 0.15]}
                      onValueChange={(value) => applyBulkStrength(value[0])}
                      min={0}
                      max={1}
                      step={0.05}
                      className="w-full"
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => applyBulkStrength(0.1)}
                        className={`text-xs ${files[0]?.strength === 0.1 ? 'border-blue-500 bg-blue-50' : ''}`}
                      >
                        Légère (10%)
                      </Button>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => applyBulkStrength(0.15)}
                        className={`text-xs ${files[0]?.strength === 0.15 ? 'border-blue-500 bg-blue-50' : ''}`}
                      >
                        Normale (15%)
                      </Button>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => applyBulkStrength(0.25)}
                        className={`text-xs ${files[0]?.strength === 0.25 ? 'border-blue-500 bg-blue-50' : ''}`}
                      >
                        Forte (25%)
                      </Button>
                    </div>
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-md p-3 mt-3">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="text-xs text-slate-700">
                          <p className="font-semibold text-slate-900 mb-1">Conseil</p>
                          <p>
                            {files[0]?.strength && files[0].strength < 0.15
                              ? "Transformation subtile - préserve davantage l'image originale"
                              : files[0]?.strength && files[0].strength > 0.2
                              ? "Transformation marquée - l'IA modifie davantage l'image"
                              : "Valeur recommandée - équilibre entre originalité et transformation"}
                          </p>
                        </div>
                      </div>
                    </div>
