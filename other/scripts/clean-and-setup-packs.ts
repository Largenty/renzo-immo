/**
 * Script pour nettoyer et recréer les packs de crédits
 * Usage: npx tsx scripts/clean-and-setup-packs.ts
 */
...
async function insertCreditPackInDatabase(
...
async function main() {
  console.log('🚀 Nettoyage et recréation des produits Stripe...\n');
  ...
  for (const pack of CREDIT_PACKS) {
    try {
      ...
      await insertCreditPackInDatabase(pack, productId, priceId);
  ...
  console.log(`   Price range: ${(CREDIT_PACKS[0].priceCents / 100).toFixed(2)}€ - ${(CREDIT_PACKS[CREDIT_PACKS.length - 1].priceCents / 100).toFixed(2)}€`);
  console.log(`\n🔗 Next steps:`);
